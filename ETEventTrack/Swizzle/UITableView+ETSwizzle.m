...
- (void)swizzled_tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    @try {
        ...
        if (cell.et_eventId.length > 0) {
            ...
            ETLog(@"点击事件控制所属控制器:%@   事件ID:%@  事件名称:%@", NSStringFromClass([[ETViewControllerUtils getViewControllerWithSubView:tableView] class]), eventId, NSStringFromSelector(action));
        } else {
            //从用户埋点配置文件中查找点击事件id
            ...
            if (controllerName.length > 0 && actionString.length > 0) {
                ...
                ETLog(@"点击事件控制所属控制器:%@   事件ID:%@  事件名称:%@", NSStringFromClass([controller class]), eventId, actionString);
            }
        }
        ...
        ETLog(@"tableView点击埋点异常: %@", exception);
    }
}
