...
#import "FFThreadSafeMutableArray.h"

#define FFFilePath @"JAStatistics/Cancel"      // 沙河存储目录
#define FFFileName @"TrackInfo"                // 沙河Json文件名称

#define dUPLOAD_INTERVAL 10.0                    // 上传埋点信息的时间间隔 (采用定时上传)

@interface FFTrackDataManager ()

@property(nonatomic, strong) FFThreadSafeMutableArray *currentArray;
...
@implementation FFTrackDataManager

dSINGLETON_FOR_CLASS(FFTrackDataManager)
...
- (void)addEventTrackData:(NSDictionary *)trackData {
    ...
    // 将埋点数据添加到数组
    [self.currentArray addObject:trackData];
}
...
- (void)uploadLocalEventTrackData {
    ...
    NSArray *loactionArray = [FFJsonUtil fromJSONString:jsonString];
...
- (void)uploadEventTrackDataWithCount:(NSUInteger)count callback:(KResponeAlertCallBack)callback {
    ...
    // 设置埋点数据
    [params setValue:uploadMutableArray forKey:@"body"];
    
    self.uploading = YES;
    @weakify(self);
    NSString *url = [NSString stringWithFormat:@"%@%@", dBASE_URL_HTTP, @"/api/burypoint/bury"];
    [[FFNetworkManager sharedInstance] postWithUrl:url postParameters:params complete:^(id JSONResponse, NSError *error) {
        @strongify(self);
        self.uploading = NO;
        NSString *status = np_objectForKPath(JSONResponse, @"code");
        NSString *msg = np_objectForKPath(JSONResponse, KResponseMessageKey);
        BOOL success = NO;
        if ([status isEqualToString:@"0000"] && error == nil) {
            ...
            if (self.currentArray.count > 0) {
                ...
                if (count >= self.currentArray.count) {
                    ...
                    if (count < self.currentArray.count) {
                        [self.currentArray removeObjectsInRange:NSMakeRange(0, count)];
                    }
                }
            }
            //清除数据
...
- (void)saveLocalEventTrackData {
    ...
    NSString *loactionString = [FFJsonUtil toJSONString:self.currentArray];
